...
public:
    ...
    void allOccurencies(const string& s, vi& a) {
        ...
        vi used(trie_size, 0);
        forn(i, 0, s.size()) {
            v = nodes[v].next[s[i] - 'a'];
            int tmp = v;
            while (used[tmp] == 0) {
                used[tmp] = 1;
                for (auto& j : nodes[tmp].term) a[j] = i;
...
int main() {
    ...
    trie.buildAvt();
    rev_trie.buildAvt();
    string text, text_rev;
    cin >> text;
    text_rev = text;
    reverse(all(text_rev));
    vc<vi> ans(2, vi(n, -1));
    trie.allOccurencies(text, ans[0]);
    rev_trie.allOccurencies(text_rev, ans[1]);
    forn(i, 0, n) {
        cout << (ans[0][i] == -1 ? -1 : ans[0][i] - szs[i] + 1) << ' ';
        cout << (ans[1][i] == -1 ? -1 : int(text.size()) - ans[1][i] - 1) << '\n';
    }
}