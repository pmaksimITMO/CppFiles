...
int main() {
    ...
    vi lcp = sa.calcLCP();
    int ansLen = -1, ansId = 0, len = int(min(x.size(), y.size()));
    forn(i, 0, lcp.size()) {
        bool tmp_less = sa.perm[i] < len, nxt_less = sa.perm[i + 1] < len;
        if ((tmp_less && !nxt_less || !tmp_less && nxt_less) && lcp[i] > ansLen) {
            ansLen = lcp[i];
            ansId = sa.perm[i];
        }
    }
    cout << sa.s.substr(ansId, ansLen);
}