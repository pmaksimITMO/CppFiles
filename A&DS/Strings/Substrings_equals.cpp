...
int main() {
    ...
    cin >> s;
    int n = int(s.size());
    vll h1(n + 1, 0), p1(n, 1), h2(n + 1, 0), p2(n, 1);
    forn(i, 1, n) {
        p1[i] = (p1[i - 1] * P) % MOD;
        p2[i] = (p2[i - 1] * P2) % MOD2;
    }
    ...
    while (q--) {
        int l, r, x, y;
        cin >> l >> r >> x >> y;
        ll t11 = (MOD + h1[r] - (h1[l - 1] * p1[r - l + 1]) % MOD) % MOD;
        ll t12 = (MOD + h1[y] - (h1[x - 1] * p1[y - x + 1]) % MOD) % MOD;
        ll t21 = (MOD2 + h2[r] - (h2[l - 1] * p2[r - l + 1]) % MOD2) % MOD2;
        ll t22 = (MOD2 + h2[y] - (h2[x - 1] * p2[y - x + 1]) % MOD2) % MOD2;
        cout << (t11 == t12 && t21 == t22 ? "Yes" : "No") << '\n';
    }
}